    ...
    if delete:
        ...
        if full:
            mongo_connection[config['project_name'] + '-pipeline'].update_many({function_id: 2},
                                                                               {'$set': {function_id: 0}})
            mongo_connection[config['project_name'] + '-pipeline-s3'].update_many({function_id: True},
                                                                               {'$unset': {function_id: '',
                                                                                            function_id + '_sb': ""}})
...
def _state_function(function_id):
    ...
    main_state = (
        collection.count_documents({function_id: {'$exists': False}}),
        collection.count_documents({function_id: False}),
        collection.count_documents({function_id: True})
    )
    s3_state = (
